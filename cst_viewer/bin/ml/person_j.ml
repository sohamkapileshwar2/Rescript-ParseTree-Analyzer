...
let write_nestedName : _ -> nestedName -> _ = (
  fun ob (x : nestedName) ->
    Buffer.add_char ob '{';
    ...
    if !is_first then
      ...
      Buffer.add_char ob ',';
      Buffer.add_string ob "\"firstName\":";
    (
...
let string_of_nestedName ?(len = 1024) x =
  let ob = Buffer.create len in
  write_nestedName ob x;
  Buffer.contents ob
let read_nestedName = (
  fun p lb ->
    Yojson.Safe.read_space p lb;
    Yojson.Safe.read_lcurl p lb;
    let field_firstName = ref (None) in
    let field_lastName = ref (None) in
    try
      Yojson.Safe.read_space p lb;
      ...
      let f =
        fun s pos len ->
          ...
          match len with
            | 8 -> (
                if String.unsafe_get s pos = 'l' && String.unsafe_get s (pos+1) = 'a' && String.unsafe_get s (pos+2) = 's' && String.unsafe_get s (pos+3) = 't' && String.unsafe_get s (pos+4) = 'N' && String.unsafe_get s (pos+5) = 'a' && String.unsafe_get s (pos+6) = 'm' && String.unsafe_get s (pos+7) = 'e' then (
                  1
                )
                else (
                  -1
    ...
    with Yojson.End_of_object -> (
        (
          {
            firstName = (match !field_firstName with Some x -> x | None -> Atdgen_runtime.Oj_run.missing_field p "firstName");
            lastName = (match !field_lastName with Some x -> x | None -> Atdgen_runtime.Oj_run.missing_field p "lastName");
          }
         : nestedName)
      )
)
let nestedName_of_string s =
  read_nestedName (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
...
let write_constant = (
  fun ob x ->
    match x with
      | `Pconst_integer x ->
        Buffer.add_string ob "<\"Pconst_integer\":";
        (
          fun ob x ->
            Buffer.add_char ob '(';
            (let x, _ = x in
        ...
        Buffer.add_char ob '>'
      | `Pconst_char x ->
        Buffer.add_string ob "<\"Pconst_char\":";
        (
          Yojson.Safe.write_int
        ) ob x;
        Buffer.add_char ob '>'
      | `Pconst_string x ->
        Buffer.add_string ob "<\"Pconst_string\":";
        (
          fun ob x ->
            Buffer.add_char ob '(';
            (let x, _ = x in
        ...
        Buffer.add_char ob '>'
      | `Pconst_float x ->
        Buffer.add_string ob "<\"Pconst_float\":";
        (
          fun ob x ->
            Buffer.add_char ob '(';
            (let x, _ = x in
...
let string_of_constant ?(len = 1024) x =
  let ob = Buffer.create len in
  write_constant ob x;
  Buffer.contents ob
let read_constant = (
...
let constant_of_string s =
  read_constant (Yojson.Safe.init_lexer ()) (Lexing.from_string s)
